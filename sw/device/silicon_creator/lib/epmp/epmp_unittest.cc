 ...
 */
struct PmpCsrs {
  std::vector<uint8_t> pmpcfg = std::vector<uint8_t>(kEpmpNumRegions, 0);
  std::vector<uint32_t> pmpaddr = std::vector<uint32_t>(kEpmpNumRegions, 0);
  uint64_t mseccfg = 0;

  epmp_state_t State() const {
    epmp_state_t state;
    for (int i = 0; i < kEpmpNumRegions; ++i) {
      bitfield_field32_t field = {.mask = 0xff,
                                  .index = (static_cast<uint32_t>(i) % 4) * 8};
      state.pmpcfg[i / 4] = bitfield_field32_write(
          state.pmpcfg[i / 4], field, static_cast<uint32_t>(pmpcfg.at(i)));
    }
    for (int i = 0; i < kEpmpNumRegions; ++i) {
...
static void ExpectPmpcfgRead(const PmpCsrs &expect) {
  // Pack configuration entries into registers.
  std::vector<uint32_t> cfg(kEpmpNumRegions / 4, 0);
  for (uint32_t i = 0; i < expect.pmpcfg.size(); ++i) {
    bitfield_field32_t field = {.mask = 0xff, .index = (i % 4) * 8};
    cfg[i / 4] = bitfield_field32_write(
        cfg.at(i / 4), field, static_cast<uint32_t>(expect.pmpcfg.at(i)));
  }

  // Set expected values to be read from registers.
  for (int i = 0; i < cfg.size(); ++i) {
    EXPECT_CSR_READ(kPmpcfgMap.at(i), cfg.at(i));
  }
}
...
static uint32_t napot(epmp_region_t region) {
  return (uint32_t)((region.start >> 2) |
...
TEST_F(EpmpCheckTest, Default) {
  ...
  EXPECT_TRUE(epmp_state_check(&state_));
}
...
TEST_F(EpmpCheckTest, ErrorPmpaddr) {
  ...
  // Check should fail.
  ExpectReadState(bad);
  EXPECT_FALSE(epmp_state_check(&state_));
}
