 ...
 *   2. The shadow registers in `epmp_state_t` used to verify the CSRs
 *
 * Every time the hardware configuration is updated the shadow registers
 * must also be updated. The hardware configuration is usually interacted
 * with directly using the CSR library or assembly whereas the shadow
 * register state should normally be modified using configuration functions
 ...
 * initialized) with the shadow register state used to double check the
 * configuration as required.
 */

/**
 * Initialise the ePMP shadow register state to reflect the hardware
 * configuration expected at entry to the mask ROM C code.
 *
