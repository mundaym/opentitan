...
/**
 * Toggle for Rule Locking Bypass (mseccfg.RLB).
 */
typedef enum epmp_rlb_toggle {
  kEpmpRlbToggleDisabled = 0, /**< mseccfg.RLB = 0 */
  kEpmpRlbToggleEnabled,      /**< mseccfg.RLB = 1 */
} epmp_rlb_toggle_t;
 ...
 * (mseccfg.RLB) is set.
 *
 * When Machine Mode Lockdown is disabled (mseccfg.MML is unset) the
 ...
 *
 * TODO: MML mode could be supported by adding additional enums. That
...
typedef enum epmp_permissions {
  ...
  // TODO: do we actually want to have unlocked rules? Could maybe remove
  // these enums since MML will probably be enabled when in S/U-mode anyway.
...
/**
 * Toggle the Rule Locking Bypass (RLB) feature.
 *
 ...
 * RLB. Note that this function does *not* check if the RLB can still be
 * modified.
 *
 * @param state State to update.
 * @param toggle Whether RLB should be enabled or disabled.
 * @return `epmp_result_t`
 */
epmp_result_t epmp_toggle_rule_locking_bypass(epmp_state_t *state,
                                              epmp_rlb_toggle_t toggle);
 ...
 *   2. pmpcfg0-pmpcfg15
 *   3. mseccfg (RLB bit only)
 *
 * This means that any changes to the Rule Locking Bypass (mseccfg.RLB)
 * setting will not be applied until *after* the PMP entries have all
 * been updated. Therefore it is necessary to check that RLB is enabled
 * before using this function to update any locked entries.
 *
 ...
 * check that `mseccfg.RLB` matches the `mseccfg_rlb` field, `mseccfg.MML` = 0
 * and `mseccfg.MMWP` = 1.
 *
 * TODO: make state optional (so you can just check mseccfg)?
 * TODO: make expected mseccfg value more configurable?
 *
